...
```text
ให้ส่งของขวัญไปเรื่อยด้วยจำนวน K ที่รับเข้ามาจนกว่าจะถึงคนที่1หรือคนที่ T โดยส่งวนในวงกลม N
```
...
```text
Step 1:สร้างลูป
Step 2:บวกตำแหน่งที่ละK
Step 3:เช็คว่าเป็น 1 หรือ T หรือไม่
```
...
```text
ก็วิธ๊เดียวกันแต่ต้องแก่วิธีการใช้ method list --> set
แก้ logic คำนวณนิดหน่อย
```
...
```text
เพราะต้องการเทส กรณีที่โค้ดเริ่มลูปใหม่ที่1 แต่ในโค้ดเดิมเกิดerror เนื่องจาก p % N == 0
```
